// management-app/src/components/Navbar.js
    ...
    >
      ...
      <div className="flex justify-center w-full md:w-auto md:ml-8">
        <Link to="/menu" className="flex items-center space-x-2">
          ...
          <span className="text-xl font-bold mt-1"> | Café Vendor</span>
        </Link>
      </div>
      ...
      {token && (
        <>
          ...
          <div className={`md:hidden absolute top-14 left-64 right-0 bg-[#003D5D] shadow-md rounded-b-xl ${isOpen ? 'block' : 'hidden'}`}>
            ...
            <Link to="/menu" className="py-2 hover:underline" onClick={() => setIsOpen(false)}>Menu</Link>
              <Link to="/orders" className="py-2 hover:underline" onClick={() => setIsOpen(false)}>Orders</Link>
